...
function parseSituation(input: CompanionIntakeInput): ParsedSituation {
  ...
  const correctionSignal = /not accurate|that'?s wrong|no, actually|not what happened/i.test(
    input.situationText,
  );
...
function synthesize(
  ...
  const betweenDynamic = context.patterns.includes("pursue_withdraw_cycle")
    ...
    : context.patterns.includes("criticism_defensiveness_loop")
      ...
      : "The interaction may have shifted from one event into broader meaning too quickly.";

  return {
    userSideHypothesis: `You may be trying ${parsed.inferredNeed}.`,
    otherSideHypothesis:
      "They may be trying to reduce pressure or uncertainty, which can look distant even if they still care.",
    betweenDynamic,
...
function buildEvidence(
  ...
  if (context.corrections.length > 0 || parsed.correctionSignal) {
    evidence.push(
      `User correction signal: ${context.corrections[0] ?? "You indicated parts of prior interpretation may be inaccurate."}`,
    );
  }
...
function followUpActions(synthesis: CompanionStructuredSynthesis): CompanionReasoningResult["followUpActions"] {
  return [
    {
      type: "show_evidence",
      label: "Show me what this is based on",
      payload: { mode: "evidence" },
    },
    {
      type: "rephrase",
      label: "Help me say it another way",
      payload: { style: "calm-clarity" },
    },
    {
