import type { InsightApiResponse, SimulationApiResponse } from "../../platform/src";
...
function inferPatternCandidates(text: string) {
  if (containsAny(text, [/silent/i, /pull(?:ed|ing)? back/i, /distance/i, /withdrew?/i])) {
    return [{ name: "pursue-withdraw", confidence: "medium" as const }, { name: "timing mismatch", confidence: "low" as const }];
  }
  if (containsAny(text, [/critic/i, /defens/i, /argu/i, /blame/i])) {
    return [{ name: "criticism-defensiveness", confidence: "medium" as const }, { name: "repair pressure", confidence: "low" as const }];
  }
  if (containsAny(text, [/family/i, /parent/i, /mother/i, /father/i, /sibling/i])) {
    return [{ name: "family role reactivation", confidence: "medium" as const }, { name: "timing mismatch", confidence: "low" as const }];
  }
...
function makeOpenAIRequestConfig(config?: ReasoningProviderConfig): Pick<StructuredOutputRequest, "apiKey" | "model" | "provider" | "enableModelGeneration"> {
  return {
...
export async function generateInsightResponseWithProvider(request: string, config?: ReasoningProviderConfig): Promise<InsightApiResponse> {
  try {
    const providerResponse = await requestStructuredOutput<InsightApiResponse>({
      name: "relationship_insight",
      instructions:
        "Give a calm, non-diagnostic, non-mystical relationship insight grounded in observable interaction patterns, timing, and repetition. Do not assign fixed personality labels or imply certainty. Treat symbolic priors only as weak context, never as certainty. Return only the schema.",
      request,
      schema: insightResponseSchema as Record<string, unknown>,
      ...makeOpenAIRequestConfig(config),
    });
    return sanitizeInsightResponse(providerResponse ?? generateInsightResponse(request));
  } catch (error) {
    if (error instanceof OpenAIRefusalError) {
      return safeInsightRefusal();
    }
    return sanitizeInsightResponse(generateInsightResponse(request));
  }
}

export async function generateSimulationResponse(request: string, config?: ReasoningProviderConfig): Promise<SimulationApiResponse> {
  try {
    const providerResponse = await requestStructuredOutput<SimulationApiResponse>({
      name: "relationship_simulation",
      instructions:
        "Return three calm, grounded simulation branches tied to observable interaction patterns. Avoid diagnosis, fixed labels, certainty, or therapy-bot language. Return only the schema.",
      request,
      schema: simulationResponseSchema as Record<string, unknown>,
      ...makeOpenAIRequestConfig(config),
    });
    return sanitizeSimulationResponse(providerResponse ?? fallbackSimulationResponse(request));
  } catch (error) {
    if (error instanceof OpenAIRefusalError) {
      return safeSimulationRefusal();
    }
    return sanitizeSimulationResponse(fallbackSimulationResponse(request));
  }
}
